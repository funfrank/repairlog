module MySearch
  ...
  def dynamic_conditions_clauses
    ...
    y[I18n.locale.to_s]['activerecord']['where'][module_key].each do |p|
      #1.attr_accessor
      if p[1] == 'datetime'
        b= %Q{ def #{p[0]}_start  
                 @#{p[0]}_start  
  ...
  def search(attrs={})
    cs = conditions    
    if attrs[:where] 
      cs[0] = case when cs[0].length > 0 
                attrs[:where] << ' AND ' + cs[0] 
              else
                attrs[:where]
              end
    end
    if attrs[:group] 
      self.class.where(cs).group(attrs[:group]).select(attrs[:select])
    else
      self.class.find(:all, :conditions => cs, :order => orders)    
    end
  end


  def search_with_paginate(page, per_page=20)
    self.class.paginate :page => page, :per_page => per_page,
                        :conditions => conditions, :order => orders
  end
